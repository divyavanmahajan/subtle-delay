...
dbquery.initialize = function(dbConfig) {
    ...
    var promise=new Promise(function (fulfil, reject) {
		...
		if(err) {
			util.winston.error(moment().format()+":dbquery:check_connection: Cannot create connection pool to database with user:" + config.user + " and connection string:" + config.connectString);
			util.winston.error(moment().format()+":"+err.message);
			process.exit(-1);
			return;
		}
		connectionpool=pool;
		connectionpool.getConnection(
		  function(err, connection) {
		    if (err) {
			util.winston.error(moment().format()+":dbquery:check_connection: Cannot logon to database with user:" + config.user + " and connection string:" + config.connectString);
			util.winston.error(moment().format()+":"+err.message);
			process.exit(-1);
			return;
		    } else {
			util.winston.info("dbquery:check_connection: Passed");
...
dbquery.query = function(querystring, params, options ) {
    ...
    if (typeof(connectionpool)=="undefined") {
        util.winston.error(moment().format()+":db_query: Connection pool is not ready.");
        r({message:"Connection pool is not ready"});
    } else {
        //util.winston.info(moment().format()+":db_query: Connection pool is ready.");
    }
    connectionpool.getConnection(
        function(err, connection) {
            if (err) {
                util.winston.error("dbquery:query: Cannot logon to database with user:" + config.user + " and connection string:" + config.connectString);
                util.winston.error(err.message);
		r(err);
                return;
            }
            //util.winston.debug(moment().format()+":db_query: Querying");

            connection.execute(querystring, params, options,
                function(err, result) {
                    //util.winston.debug(moment().format()+":db_query: Query results");
                    if (err) {
                        util.winston.error(moment().format()+":dbquery:query: Cannot query database with query:" + querystring);
                        util.winston.error(err.message);
                        doRelease(connection).then(function() { 
				r(err);
			},r);
                    ...
                    doRelease(connection).then(function() { 
                        //util.winston.debug(moment().format()+":db_query: Release");
try{
                        f({"query":querystring,"metadata":result.metaData, "rows":result.rows});
} catch(err1) {r(err1);}
                        //util.winston.debug(moment().format()+":db_query: Release done");
		    },r);
                });
        });
    });
...
dbquery.dump_to_console = function(querystring,metadata, rows) {
    util.winston.info(JSON.stringify(result.metaData));
    util.winston.info(JSON.stringify(result.rows));
};
/**
...
dbquery.query_staging_table = function(tablename, object_name, for_time) {
    //util.winston.info('dbquery:query_staging_table - for :' + tablename + ":" ,for_time.format());
    ...
    //util.winston.info('query_staging_table:' + querystring + "\n\t" + JSON.stringify(params));
	...
	var rows=res.rows;
	//util.winston.info(moment().format()+":db_query: Process results "+JSON.stringify(rows));
        var count = rows.length;
	//util.winston.info(moment().format()+":db_query: Process results "+count);
        util.updateFirebase(range[1], object_name, count);
	//util.winston.info(moment().format()+":db_query: returning from promise");
        return {"tablename":tablename, "query":query, "metadata":metadata, "rows":rows};
    },function(err) {
        util.winston.error(moment().format()+": Error during query:"+JSON.stringify(err));
    });
};
/**
 * Query the staging table for a single entry
 * @param {String} tablename Database table to query
 * @param {String} object_name Name of the object in Firebase - used to update the count.
 * @param {String} Saleforce ID for the object
 * @param {function(query,metadata,rows)} process_function callback to process the data.
 * @return Promise
 */
dbquery.query_staging_table_single = function(tablename, object_name, sf_id)
{
    var querystring = "SELECT SFDCID,LASTMODIFIEDDATE,ROW_UPD_DT AS ROWCOUNT "
        + " " 
        + " FROM " + tablename
        + " WHERE SFDCID = :sf_id";
    var params = {
        sf_id: { val: sf_id, dir: oracledb.BIND_IN, type: oracledb.STRING },
    };
    var options = {
        /*        fetchInfo: {
                    "ROWCOUNT": { type: oracledb.DEFAULT }
	...
	var metadata=res.metadata;
	var rows=res.rows;
	//util.winston.debug(moment().format()+":db_query: Process results "+JSON.stringify(rows));
        var count = rows.length;
	//util.winston.debug(moment().format()+":db_query: returning from promise");
        return {"tablename":tablename, "query":query, "metadata":metadata, "rows":rows};
    },function(err) {
        util.winston.error(moment().format()+": Error during query:"+JSON.stringify(err));
    });
...
function doRelease(connection) {
    ...
    connection.release(
        function(err) {
            if (err) {
                util.winston.error(moment().format()+":dbquery:doRelease: Cannot release a connection" + err.message);
		reject(err);
            }
	    fulfill();
        });
