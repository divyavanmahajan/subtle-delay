...
dbquery.query_staging_table = function(tablename, object_name, for_time) {
    ...
    var querystring = "SELECT COUNT(*) AS ROWCOUNT FROM " + tablename
        + " WHERE row_upd_dt >= To_date(:fromdate, 'yyyy-mm-dd hh24:mi:ss') "
        + " AND row_upd_dt <= To_date(:todate, 'yyyy-mm-dd hh24:mi:ss')";
    var params = {
        fromdate: { val: range[0].format('YYYY-MM-DD HH:mm:ss'), dir: oracledb.BIND_IN, type: oracledb.STRING },
        todate: { val: range[1].format('YYYY-MM-DD HH:mm:ss'), dir: oracledb.BIND_IN, type: oracledb.STRING }
    };
    var options = {
        /*        fetchInfo: {
...
dbquery.query_all_staging = function(for_time) {
    dbquery.query_staging_table('s_servicecontract', 'WCServiceContract', for_time);
