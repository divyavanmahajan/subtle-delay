            ...
            connection.execute(querystring, params, options,
                function(err, result) {
                    ...
                    if (typeof (process_function) == 'function') {
                        process_function(querystring,result.metaData, result.rows);
                    } else {
 ...
 */
dbquery.query_staging_table = function(tablename, object_name, for_time, process_function) {
    ...
    dbquery.query(querystring, params, options, function(query,metadata, rows) {
        var count = rows.length;
        util.updateFirebase(range[1], object_name, count);
        if (typeof (process_function) == 'function') {
            process_function(tablename, query, metadata, rows);
        } else {
            console.log('dbquery:querystagetable - returns :' + count + ':'
                + range[0].format('YYYY-MM-DDTHH:mm:ss') + ':' + range[1].format('YYYY-MM-DDTHH:mm:ss'));
            console.log(metadata);
            console.log(rows);
        }
    });
};
dbquery.query_all_staging = function(for_time, process_function) {
    dbquery.query_staging_table('sfstage.s_servicecontract', 'WCServiceContract', for_time, process_function);
