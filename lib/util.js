...
util.initialize = function(config) {
    ...
    util.baseRef.authWithCustomToken(token,function(err,authData) {
  	if (err) {
	    console.log(moment().format()+":"+"Error logging into Firebase. Exiting");
	    process.exit(-1);
	} else {
	    console.log(moment().format()+":"+"Login to Firebase succeeded");
	}
    });
    util.OAuthRef = new Firebase(config.oauthtoken_url);
    util.OAuthRef.authWithCustomToken(token,function(err,authData) {
  	if (err) {
	    console.log(moment().format()+":Error logging into Firebase for the OAuth token. Continuing");
	} else {
	    console.log(moment().format()+":"+"Login to Firebase for OAuth token url succeeded");
	}
    });
}
/**
...
util.updateFirebase = function(timestamp,object_name,value) {
    ...
    //console.log(moment().format()+":"+'   Firebase_util:'+key+":"+object_name+":"+value);
}
...
util.updateFirebaseString = function(keystring,object_name,value) {
    ...
    updateMsg[object_name]=value;
    tsRef.update(updateMsg);    
    //console.log(moment().format()+":"+'   Firebase_util:'+keystring+":"+object_name+":"+value);
}
...
util.removeFirebaseString = function(keystring) {
    ...
    tsRef.remove().then(function() {
	    // console.log(moment().format()+":"+'   Firebase_util:remove'+keystring);
    });
}
