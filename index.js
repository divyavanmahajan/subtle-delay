...
monitor.initialize = function(dbconfig) {
        ...
        //w.debug(JSON.stringify(monitor.config));
...
function poll(timestamp,table,objectname) {
    ...
    monitor.dbquery.query_staging_table(table, 'DB_'+objectname,timestamp).then(function(res) {
		...
		w.error(moment().format()+":Error during query");
	}
	...
	var rows = res.rows;
        dbchanges = rows;
        //w.debug('dbquery:query_all_staging - returns :' + tablename + ":" + rows.length);
        //w.debug(metadata);
        //w.debug(rows);
        ...
        monitor.sfquery.getObjectsChanged(objectname,'SF_'+objectname, timestamp, function(err,sf_object, object_name, for_time, query, result) {
            // w.debug('sfquery:getObjectsChanged returns');
            sfchanges = result;
	    if (err==null) {
		//w.debug('Salesforce returns : ' + result.totalSize);
		compareChanges(timestamp,dbchanges,sfchanges,sf_object);
	    } else {
		//w.debug('Salesforce returns : '+ err);
		compareChanges(timestamp,dbchanges,null,sf_object);
	    }
          }
        );
    },function(err) {
        w.error(moment().format()+": Error during polling:"+JSON.stringify(err));
    });
